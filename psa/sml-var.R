...
covmat <- cov.x1.x2(dat$x.e, dat$x.e, pars$v_sigma, pars$v_theta, 2) + diag(pars$v_nugget, dim(dat$x.e)[1])
...
first.order <- function(N, dat, Beta, precmat, xrange = c(-1.7,1.7)){
  ...
  crosscov.M <- cov.x1.x2(M, dat$x.e, pars$v_sigma, pars$v_theta, 2)
  mu.M <- cbind(1, M)%*%Beta
  ...
  for(j in 1:p){
    M2 <- M.prime
    M2[,j] <- M[,j]
    crosscov.M2 <- cov.x1.x2(M2, dat$x.e, pars$v_sigma, pars$v_theta, 2)
